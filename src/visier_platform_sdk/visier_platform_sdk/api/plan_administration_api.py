...
class PlanAdministrationApi:
    ...
    def patch_plan(
        ...
        Make partial changes to a scenario in an existing plan. Returns scenario update results.   Retrieve the object definition using `GET` before patching. Use the `GET` response in your `PATCH` request. To retrieve plan IDs and scenario IDs, see `GET /v1/planning/model/plans`. Plans containing `parentPlanUuid` are subplans.   Use `PATCH` to update specific scenario fields without changing omitted fields.
    ...
    def plan_action(
        ...
        Make partial changes to existing plans. Returns plan update results.   Retrieve the object definition using `GET` before patching. Use the `GET` response in your `PATCH` request. To retrieve main plan or subplan IDs, see `GET /v1/planning/model/plans`. Plans containing `parentPlanUuid` are subplans.   Use `PATCH` to update specific plan fields without changing omitted fields.
    ...
    def _plan_action_serialize(
        ...
        return self.api_client.param_serialize(
            method='PATCH',
            resource_path='/v1/planning/data/plans/{planId}',
            path_params=_path_params,
            query_params=_query_params,
            header_params=_header_params,
            body=_body_params,
