...
class QueryTimeIntervalDTOWithDynamicDateFrom(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["dynamicDateFrom", "intervalPeriodType", "intervalPeriodCount", "direction", "shift"]

    @field_validator('dynamic_date_from')
    def dynamic_date_from_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SOURCE', 'COMPLETE_PERIOD', 'CURRENT_DATE', 'CURRENT_DATE_COMPLETE_PERIOD']):
            raise ValueError("must be one of enum values ('SOURCE', 'COMPLETE_PERIOD', 'CURRENT_DATE', 'CURRENT_DATE_COMPLETE_PERIOD')")
        return value
