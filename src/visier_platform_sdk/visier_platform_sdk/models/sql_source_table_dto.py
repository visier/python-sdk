...
from visier_platform_sdk.models.sql_source_materialized_view_config_dto import SqlSourceMaterializedViewConfigDTO
...
class SqlSourceTableDTO(BaseModel):
    ...
    exclude_from_auto_processing: Optional[StrictBool] = Field(default=None, description="If `true`, Visier ignores this source when checking which data categories require a processing job during a `process` action with no specified `dataCategoryIds`. Default is `false`.", alias="excludeFromAutoProcessing")
    snapshot_retention_days: Optional[StrictInt] = Field(default=None, description="The number of days to keep snapshots for this table before permanently deleting them. Must be between 1 and 180 days.  If undefined, the default is 3 days. You cannot reset the table to a timestamp older than the retention period because older snapshots are permanently deleted.", alias="snapshotRetentionDays")
    materialized_view: Optional[SqlSourceMaterializedViewConfigDTO] = Field(default=None, description="The configuration options for a materialized view. Omit or set `enabled=false` for non-materialized view tables.", alias="materializedView")
    filter_inherited_data_by_tenant: Optional[StrictBool] = Field(default=None, description="If `true`, analytic tenants only see data from inherited table data for rows where `TENANT_ID` or `CUSTOMER_ID` matches their own tenant code. Requires `canChildrenInheritData=true`.", alias="filterInheritedDataByTenant")
    __properties: ClassVar[List[str]] = ["uuid", "objectName", "basicInformation", "columns", "canChildrenInherit", "canChildrenInheritData", "excludeFromAutoProcessing", "snapshotRetentionDays", "materializedView", "filterInheritedDataByTenant"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "excludeFromAutoProcessing": obj.get("excludeFromAutoProcessing"),
            "snapshotRetentionDays": obj.get("snapshotRetentionDays"),
            "materializedView": SqlSourceMaterializedViewConfigDTO.from_dict(obj["materializedView"]) if obj.get("materializedView") is not None else None,
            "filterInheritedDataByTenant": obj.get("filterInheritedDataByTenant")
        })
        return _obj
