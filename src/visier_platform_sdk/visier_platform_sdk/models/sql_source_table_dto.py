...
class SqlSourceTableDTO(BaseModel):
    ...
    snapshot_retention_days: Optional[StrictInt] = Field(default=None, description="The number of days to retain snapshots for this table. If not set, the global default is used.  Note: this setting limits how far back a Reset action can target. Snapshots older than the retention  period are permanently deleted by the vacuum job, so a Reset to a timestamp beyond the retention  window will fail because the target snapshot no longer exists.", alias="snapshotRetentionDays")
    __properties: ClassVar[List[str]] = ["uuid", "objectName", "basicInformation", "columns", "canChildrenInherit", "canChildrenInheritData", "excludeFromAutoProcessing", "snapshotRetentionDays"]
