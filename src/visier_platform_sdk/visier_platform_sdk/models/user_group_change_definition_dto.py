...
class UserGroupChangeDefinitionDTO(BaseModel):
    ...
    is_visible_in_features: Optional[StrictBool] = Field(default=None, description="If `true`, the user group is visible in features such as sharing an analysis with user groups or assigning user groups a Vee sample question.  If `false`, the user group is not visible to select in features.  Default is `true`.  **Note**: User group visibility is not available until Summer 2025.", alias="isVisibleInFeatures")
    visible_features: Optional[FeaturesForUserGroupDTO] = Field(default=None, description="The features in which the user group is visible.  If not defined, the user group is visible in all features.  If `isVisibleInFeatures` is `false`, this field is ignored.  **Note**: User group visibility is not available until Summer 2025.", alias="visibleFeatures")
    __properties: ClassVar[List[str]] = ["userGroupId", "displayName", "description", "permissionIds", "users", "tenantCode", "projectId", "isVisibleInFeatures", "visibleFeatures"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of visible_features
        if self.visible_features:
            _dict['visibleFeatures'] = self.visible_features.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "isVisibleInFeatures": obj.get("isVisibleInFeatures"),
            "visibleFeatures": FeaturesForUserGroupDTO.from_dict(obj["visibleFeatures"]) if obj.get("visibleFeatures") is not None else None
        })
        return _obj
