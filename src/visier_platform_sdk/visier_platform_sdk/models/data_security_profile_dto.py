...
class DataSecurityProfileDTO(BaseModel):
    ...
    """ # noqa: E501
    analytic_object_id: Optional[StrictStr] = Field(default=None, description="The unique identifier of the analytic object assigned data security in this permission.", alias="analyticObjectId")
    property_set_config: Optional[PropertySetConfigDTO] = Field(default=None, description="The data access for an analytic object's properties.", alias="propertySetConfig")
    all_data_point_access: Optional[StrictBool] = Field(default=None, description="If `true`, the permission grants access to the entire population. If `false`, define `memberFilterConfigs` to set custom population access. Default is `false`.", alias="allDataPointAccess")
    ...
    analytic_object_status: Optional[StrictStr] = Field(default=None, description="The analytic object's validity status. Valid values:  * `Valid`: The object exists and has loaded data.  * `NoData`: The object exists but doesn't have loaded data.  * `NotFound`: The object doesn't exist.", alias="analyticObjectStatus")
    shareable_security_filters: Optional[List[ShareableSecurityFilterDTO]] = Field(default=None, description="The shareable security filters linked to this item in the permission. You can link  shareable security filters in multiple permissions and manage them through  `/v1/admin/security-filters`. Each linked filter defines population access in  the same way as a filter in `memberFilterConfigs`. You can combine both filter types.   **Note**:  - The `analyticObjectId` of the linked filter must match the `analyticObjectId` of this item.  - On update, omitting `shareableSecurityFilters` unlinks all previously linked security filters from this item. This action does not delete the security filters.  - You cannot define this field if `allDataPointAccess` is `true`.  - The request does not return validity statuses for linked security filters. To retrieve them, see `GET /v1/admin/security-filters`.", alias="shareableSecurityFilters")
    __properties: ClassVar[List[str]] = ["analyticObjectId", "propertySetConfig", "allDataPointAccess", "memberFilterConfigs", "inheritedAccessConfigs", "inheritedReferenceMemberFilterConfig", "analyticObjectStatus", "shareableSecurityFilters"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in shareable_security_filters (list)
        _items = []
        if self.shareable_security_filters:
            for _item_shareable_security_filters in self.shareable_security_filters:
                if _item_shareable_security_filters:
                    _items.append(_item_shareable_security_filters.to_dict())
            _dict['shareableSecurityFilters'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "shareableSecurityFilters": [ShareableSecurityFilterDTO.from_dict(_item) for _item in obj["shareableSecurityFilters"]] if obj.get("shareableSecurityFilters") is not None else None
        })
        return _obj
