...
class AggregateQueryExecutionOptionsDTO(BaseModel):
    ...
    calendar_id: Optional[StrictStr] = Field(default=None, description="The unique ID of the calendar to use in time calculations unless explicitly overridden in the calculation. If both `calendarId` and `calendarType` are specified, `calendarId` takes precedence over `calendarType`.", alias="calendarId")
