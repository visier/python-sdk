...
class ProfileAssignedForAccessibleTenantDTO(BaseModel):
    ...
    validity_start_time: Optional[StrictStr] = Field(default=None, description="An inclusive date-time when this profile is active.   `Long.Min_Value` means that `startTime` is undefined.", alias="validityStartTime")
    validity_end_time: Optional[StrictStr] = Field(default=None, description="An exclusive date-time when this profile is no longer active.   `Long.Max_Value` means that `endTime` is undefined and is equivalent to permanent access.", alias="validityEndTime")
