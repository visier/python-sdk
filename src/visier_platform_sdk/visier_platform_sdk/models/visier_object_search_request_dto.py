...
class VisierObjectSearchRequestDTO(BaseModel):
    ...
    """ # noqa: E501
    query: Optional[List[StrictStr]] = Field(default=None, description="The search string. You can provide multiple search strings. The results include Visier object headers that match each of the search strings. Search strings are case-insensitive and support partial matches. Search strings must be at least 2 characters up to a maximum of 200 characters.")
    ...
    mask: Optional[StrictStr] = Field(default=None, description="A comma-separated list of additional fields to include in the response. Valid values:  - For all objects: `Description`, `Explanation`, `Tags`  - For `METRIC` search: `analyticObjectId`, `dataStartDate`, `dataEndDate`  - For `ANALYTIC_OBJECT` search: `objectReferences`, `analyticObjectType`, `dataStartDate`, `dataEndDate`, `propertyIds`  - For `DIMENSION_MEMBER` search: `path`, `level`, `dimensionId`, `matchedSynonyms`  - For `PROPERTY` search: `dataType`   Returns `matchedSynonyms` in the `metadata` response. Contains synonyms that match the dimension member search string, ordered by relevance. Returns an empty list if none match.   For example, to get descriptions and tags in the response, set `mask=\"Description,Tags\"`.")
    include_document_id_search: Optional[StrictBool] = Field(default=None, description="If `true`, the search includes document IDs. This is not valid for `DIMENSION_MEMBER` object type. Default is `false`. Note that document ID search queries are case-sensitive.", alias="includeDocumentIdSearch")
    ...
    __properties: ClassVar[List[str]] = ["query", "objectType", "limit", "offset", "mask", "includeDocumentIdSearch", "dimensionSearchContext", "conceptSearchContext", "dimensionMemberSearchContext", "propertySearchContext"]
    ...
    def object_type_validate_enum(cls, value):
        ...
        if value not in set(['UNKNOWN', 'METRIC', 'DIMENSION', 'DIMENSION_MEMBER', 'CONCEPT', 'ANALYTIC_OBJECT', 'PROPERTY']):
            raise ValueError("must be one of enum values ('UNKNOWN', 'METRIC', 'DIMENSION', 'DIMENSION_MEMBER', 'CONCEPT', 'ANALYTIC_OBJECT', 'PROPERTY')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "mask": obj.get("mask"),
            "includeDocumentIdSearch": obj.get("includeDocumentIdSearch"),
