...
class VisierObjectSearchRequestDTO(BaseModel):
    ...
    mask: Optional[StrictStr] = Field(default=None, description="A comma-separated list of additional fields to include in the response. Valid values:  - For all objects: `Description`, `Explanation`, `Tags`  - For `METRIC` search: `analyticObjectId`, `dataStartDate`, `dataEndDate`  - For `ANALYTIC_OBJECT` search: `objectReferences`, `analyticObjectType`, `dataStartDate`, `dataEndDate`, `propertyIds`  - For `DIMENSION_MEMBER` search: `path`, `level`  - For `PROPERTY` search: `dataType`   For example, to get descriptions and tags in the response, set `mask=\"Description,Tags\"`.")
