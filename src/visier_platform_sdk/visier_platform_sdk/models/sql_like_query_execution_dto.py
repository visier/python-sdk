...
class SqlLikeQueryExecutionDTO(BaseModel):
    ...
    *  Request body for SQL-like query executions, which can be either an aggregate or list query.  For examples that distinguish between aggregate and list queries, see the `POST /v1/data/query/sql` description.
    ...
    aggregate_query_options: Optional[AggregateQueryExecutionOptionsDTO] = Field(default=None, description="Options to apply to a SQL-like aggregate query. The query is aggregate if it contains at least one metric.  `enableDescendingSpace` is not supported in SQL-like aggregate queries.", alias="aggregateQueryOptions")
    list_query_options: Optional[ListQueryExecutionOptionsDTO] = Field(default=None, description="Options to apply to a SQL-like list query. The query is list if it contains no metrics and has only properties.  `multipleTables` is not supported in SQL-like list queries.", alias="listQueryOptions")
    __properties: ClassVar[List[str]] = ["query", "aggregateQueryOptions", "listQueryOptions"]
