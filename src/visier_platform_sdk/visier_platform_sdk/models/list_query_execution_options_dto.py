...
class ListQueryExecutionOptionsDTO(BaseModel):
    ...
    """ # noqa: E501
    limit: Optional[StrictInt] = Field(default=None, description="The maximum number of entries to return. Default is to return all entries. If `page` is defined but  `limit` is not, `limit` will be set to a default value of 1000.")
    ...
    date_time_display_mode: Optional[StrictStr] = Field(default=None, description="Control how date-time values are displayed in the result set.  Default is `EPOCH`. Valid values:  * `EPOCH`: The number of elapsed milliseconds since January 1, 1970 in UTC timezone.  * `DATETIME`: The date-time value displayed in `yyyy-MM-dd HH:mm:ssZZ` format.", alias="dateTimeDisplayMode")
    calendar_id: Optional[StrictStr] = Field(default=None, description="The unique identifiers of the calendar to use in time calculations unless explicitly overridden in the calculation. If both `calendarId` and `calendarType` are specified, `calendarId` takes precedence over `calendarType`.", alias="calendarId")
    __properties: ClassVar[List[str]] = ["limit", "queryMode", "omitHeader", "calendarType", "currencyConversionMode", "currencyConversionDate", "page", "multipleTables", "currencyConversionCode", "recordMode", "dateTimeDisplayMode", "calendarId"]
