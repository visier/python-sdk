...
class ServicingPublicapiTransfersAssignRevokePermissionsRequestDTO(BaseModel):
    """
    Set permissions request  List of permissionId with assign to user Ids
    """ # noqa: E501
    permissions: Optional[List[ServicingPublicapiTransfersAssignRevokePermissionRequestDTO]] = Field(default=None, description="A list of objects representing the permissions to assign to or remove from users.")
    target_project_for_tenants_list: Optional[AdminTransfersTargetProjectForTenantsListDTO] = Field(default=None, description="Administrating tenants can specify the tenants and projects in which to assign permissions to users or remove permissions from users. Specify one `projectId` per `tenantCode`.  If omitted, the request is immediately published to production or applied to the `ProjectID` in the request header, if available, for the administrating tenant or TargetTenantID, if available.", alias="targetProjectForTenantsList")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ServicingPublicapiTransfersAssignRevokePermissionsRequestDTO from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ServicingPublicapiTransfersAssignRevokePermissionsRequestDTO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "permissions": [ServicingPublicapiTransfersAssignRevokePermissionRequestDTO.from_dict(_item) for _item in obj["permissions"]] if obj.get("permissions") is not None else None,
            "targetProjectForTenantsList": AdminTransfersTargetProjectForTenantsListDTO.from_dict(obj["targetProjectForTenantsList"]) if obj.get("targetProjectForTenantsList") is not None else None
        })
        return _obj
