...
class AdminPublicapiTransfersUserGroupChangeUsersDTO(BaseModel):
    ...
    manually_excluded_ids: Optional[AdminPublicapiTransfersElementIDsDTO] = Field(default=None, description="Excludes specified user IDs from the user group.  * You can manually exclude users if `includeAllUsers` is `true` or if `dynamicFilterDefinition` is defined.  * Excluded IDs must not overlap with user IDs in `manuallyIncludedIds`.", alias="manuallyExcludedIds")
    manually_included_ids: Optional[AdminPublicapiTransfersElementIDsDTO] = Field(default=None, description="Includes specified user IDs in the user group.  * May be combined with `dynamicFilterDefinition`.  * Omit if `includeAllUsers` is `true`.", alias="manuallyIncludedIds")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AdminPublicapiTransfersUserGroupChangeUsersDTO from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AdminPublicapiTransfersUserGroupChangeUsersDTO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "dynamicFilterDefinition": AdminPublicapiTransfersUserGroupFiltersDTO.from_dict(obj["dynamicFilterDefinition"]) if obj.get("dynamicFilterDefinition") is not None else None,
            "includeAllUsers": obj.get("includeAllUsers"),
            "manuallyExcludedIds": AdminPublicapiTransfersElementIDsDTO.from_dict(obj["manuallyExcludedIds"]) if obj.get("manuallyExcludedIds") is not None else None,
            "manuallyIncludedIds": AdminPublicapiTransfersElementIDsDTO.from_dict(obj["manuallyIncludedIds"]) if obj.get("manuallyIncludedIds") is not None else None
        })
        return _obj
